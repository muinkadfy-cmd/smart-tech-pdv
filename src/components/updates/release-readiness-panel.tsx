  ...
  if (!snapshot) {
    return (
      <Card className="border-white/80 bg-white/90">
        <CardHeader>
          <CardTitle>Prontidao do release</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="empty-state-box">
            Carregando sinais do runtime, updater e ambiente atual para orientar a proxima release.
          </div>
        </CardContent>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        <div className={`rounded-3xl border px-5 py-4 ${tone.ring}`}>
          <div className="flex items-end justify-between gap-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-slate-500">Score atual</p>
              <p className={`mt-2 font-display text-4xl font-semibold ${tone.accent}`}>{snapshot.score}%</p>
            </div>
            <p className="max-w-xs text-right text-sm leading-6 text-slate-600">{snapshot.headline}</p>
          </div>
        </div>
        <div className="grid gap-3">
          {snapshot.checks.map((check) => {
            ...
            return (
              <div className="rounded-2xl bg-secondary/40 p-4" key={check.id}>
