import { Badge } from "@/components/ui/badge";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import type { SupplierPerformanceItem } from "@/features/purchases/purchase.service";

export function SupplierPerformancePanel({ items }: { items: SupplierPerformanceItem[] }) {
  return (
    <Card className="border-white/80 bg-white/90">
      <CardHeader>
        <CardTitle>Performance de fornecedores</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        {items.map((item) => (
          <div className="rounded-2xl bg-secondary/45 p-4" key={item.id}>
            <div className="flex items-start justify-between gap-3">
              <p className="font-semibold text-slate-950">{item.name}</p>
              <Badge variant={item.tone === "success" ? "success" : item.tone === "warning" ? "warning" : "outline"}>{item.leadTimeLabel}</Badge>
            </div>
            <p className="mt-2 text-sm text-muted-foreground">{item.linkedProductsLabel}</p>
          </div>
        ))}
      </CardContent>
    </Card>
  );
}
