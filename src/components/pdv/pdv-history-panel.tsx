...
export function PdvHistoryPanel({ items }: { items: PdvHistoryItem[] }) {
  return (
    <Card className="executive-panel">
      <CardHeader>
        <CardTitle>Historico recente de vendas</CardTitle>
      </CardHeader>
        ...
        {items.map((item) => (
          <div className="panel-block rounded-[18px] p-3.5" key={item.id}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[14px] font-semibold text-slate-950">{item.id}</p>
                <p className="text-[12px] text-slate-600">{item.customerName}</p>
              </div>
              <p className="font-semibold text-slate-950">{item.totalLabel}</p>
            </div>
